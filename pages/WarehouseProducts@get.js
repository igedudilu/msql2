module.exports = ['/warehouse-products/:warehouse_id', async function(pool, { params }) {
    ...
    for(let i = 0; i < productsr.length; i++) {
		...
		<td>${(productsr[i].price)}</td> 
		<td>${(productsr[i].price*productsr[i].number).toFixed(2)}</td>
	...
	result += `<h1>Изделия в процессе изготовления</h1>
		...
		for(let i = 0; i < productsnr.length; i++) {
			...
			<td>${(productsnr[i].price)}</td>
			<td>${(productsnr[i].price*productsnr[i].number).toFixed(2)}</td>
	...
	const [productssp] = await pool.query('SELECT * FROM products WHERE warehouse_id= ? AND readiness=2', warehouse_id);
	result += `<h1>Списанные изделия</h1>
		...
		for(let i = 0; i < productssp.length; i++) {
			...
			<form method="post" action="/product-ready/${productssp[i].id}">
